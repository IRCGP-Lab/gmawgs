...
if args.merge_reports:
    ...
    # Pattern matches standard TB-Profiler collate output
    ...
    if not tb_files:
        ...
        # Use the first found text file (usually there is only one per batch prefix)
        tb_summary_file = tb_files[0]
        print(f"[INFO] Using TB-Profiler summary: {tb_summary_file}")
        
        tb_data = {}
        tb_cols = []
        
        # 1. Read TB-Profiler Data
        try:
            with open(tb_summary_file, "r", encoding="utf-8", errors='replace') as f:
                reader = csv.reader(f, delimiter="\t")
                tb_header = next(reader, None)
                if tb_header and len(tb_header) > 1:
                    # Exclude the first column ('sample')
                    tb_cols = tb_header[1:]
                    for row in reader:
                        if row:
                            # Map: sample_name -> [col2, col3, ...]
                            tb_data[row[0]] = row[1:]
        ...
        if tb_cols:
            ...
            # Name convention: original.gDST_summary.tsv -> original.Integrated_Report.xlsx
            ...
            try:
                ...
                if gma_header:
                    ...
                    full_header = gma_header + tb_cols
                    ...
                    for row in gma_rows:
                        ...
                        # Left Join with TB Data
                        tb_row_data = tb_data.get(sample_id, ["-"] * tb_len)
                        if len(tb_row_data) < tb_len: tb_row_data += ["-"] * (tb_len - len(tb_row_data))
                        
                        # Truncate if TB data is wider than expected due to CSV parsing errors
                        if len(tb_row_data) > tb_len: tb_row_data = tb_row_data[:tb_len]

                        # Clean all cell data before writing
                        final_row = [clean_text(c) for c in (row + tb_row_data)]
                        ws.append(final_row)
                ...
                try:
                    ...
                    with open(fallback_csv, "w", newline="", encoding="utf-8") as f:
                        ...
                        writer.writerow(gma_header + tb_cols)
                        for row in gma_rows:
                            sample_id = row[0]
                            tb_row_data = tb_data.get(sample_id, ["-"] * tb_len)
                            if len(tb_row_data) < tb_len: tb_row_data += ["-"] * (tb_len - len(tb_row_data))
                            writer.writerow(row + tb_row_data)
